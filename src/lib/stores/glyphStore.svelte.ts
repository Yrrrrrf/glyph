...
import type { AnalysisResult, JsCompilerResult } from "$lib/types/analysisTypes";
...
export function getTokenBadgeClasses(type: string): string {
  ...
  if (t.startsWith("constant") || t.startsWith("string")) return "badge-warning text-warning-content";
  if (t.startsWith("punctuation")) return "badge-outline";
  if (t.startsWith("directive")) return "badge-accent text-accent-content";
  if (t.includes("error") || t.includes("invalid")) return "badge-error text-error-content";
  return "badge-ghost";
}
...
export function getTokenTextClass(category: string): string {
    ...
    if (c.includes("instruction")) return "text-blue-600 dark:text-blue-400 font-bold";
    if (c.includes("register")) return "text-green-600 dark:text-green-400 font-bold";
    ...
    if (c.includes("directive") || c.includes("pseudo")) return "text-purple-600 dark:text-purple-400";
    if (c.includes("symbol")) return "text-base-content"; 
    if (c.includes("error") || c.includes("invalid")) return "text-red-500 underline decoration-wavy";
...
class GlyphStore {
  ...
  runAnalysis = async () => {
    ...
    try {
      // Analyze
      const rawResult = analyze_full_program(this.sourceCode) as unknown as JsCompilerResult;
