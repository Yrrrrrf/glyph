...
// Corrected to use `extra::Err` and include the required lifetime.
type ParseErr<'a> = extra::Err<Simple<'a, Token<'a>>>;

/// Parses a mnemonic token into a `Mnemonic` enum.
fn mnemonic_parser<'a>() -> impl Parser<'a, &'a [Token<'a>], Mnemonic, ParseErr<'a>> + Clone {
...
fn operand_parser<'a>() -> impl Parser<'a, &'a [Token<'a>], Operand, ParseErr<'a>> + Clone {
    // Replaced `filter_map` with the correct `select!` macro.
    select! {
        Token::Register(s) => Operand::Register(s.to_string()),
        Token::Immediate(s) => {
        ...
        Token::Label(s) => Operand::Label(s.to_string()),
    }
    .labelled("operand")
}

/// Parses a full instruction.
// Corrected the function signature with the required `'a` lifetime.
pub fn instruction_parser<'a>() -> impl Parser<'a, &'a [Token<'a>], Instruction, ParseErr<'a>> + Clone {
    mnemonic_parser()
